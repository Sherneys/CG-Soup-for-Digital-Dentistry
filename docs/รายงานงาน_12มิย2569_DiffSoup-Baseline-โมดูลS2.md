...
## ผลเพิ่มเติมตอนดึก (23:00–23:30 น.): ยกเลิก full-res → ได้ผลเทียบคู่แรก

**Baseline full-res ไปไม่รอด:** อ่าน log จริงพบว่าอยู่แค่ 27% (step 2,743) หลังรัน 17.5 ชม. ที่ ~32 วินาที/step — สาเหตุคือ **VRAM ล้น 12GB แล้ว spill ไป shared memory** ทำให้ช้าลง ~10 เท่า (ETA ที่เหลือ ~65–80 ชม.) จึงตัดสินใจ kill แล้วรันคู่เทียบที่ `--downscale 4` แทน (ต้องสร้าง `dense/images_4/` เองก่อน — loader ไม่ resize ให้)

**ผลเทียบคู่แรกของโครงการ** (buddha, 10,000 steps, 15,000 faces เท่ากัน, ฝั่งละ ~8 นาที):

| Init | PSNR (dB) | SSIM | ชนะรายวิว (จาก 9) |
|---|---|---|---|
| **Curvature-guided (ของเรา)** | **15.920** | **0.5681** | PSNR 6, SSIM 7 |
| Random (baseline) | 15.748 | 0.5516 | PSNR 3, SSIM 2 |

- ✅ `src/curvature_init.py` + `src/diffsoup_train.py` ผ่าน E2E จริงบน GPU ครั้งแรก ทุกขั้นทำงาน (load seeds → train → resample → checkpoint → eval)
- ✅ **Curvature ชนะทั้งสองตัวชี้วัด** (+0.17 dB / +0.017 SSIM) ที่ budget หลวม (15k) — ตามสมมติฐาน margin ควรกว้างขึ้นเมื่อลด budget ลงสู่ 5,000
- 📌 บทเรียนเครื่อง 12GB: ห้ามรัน full res (`--downscale 1`) — VRAM ล้นแล้วช้าแบบไม่ crash ให้เห็น / smoke test สั้น ๆ ตรวจจับปัญหานี้ไม่ได้

## ปิด S2 (เที่ยงคืน): ครบ 3 ระดับ budget — สมมติฐานยืนยัน ✅

รันเพิ่มอีก 4 ตัว (curvature/random × 10,000/5,000 faces) เงื่อนไขเดิมทุกอย่าง:
...
**ข้อสรุป S2:**
1. Curvature-guided init ชนะ random **ทุกระดับ budget** และ **margin โต ~3 เท่าเมื่องบตึง** (0.17 → 0.44–0.51 dB) — สมมติฐานหลักของโครงการยืนยันด้วยตัวเลขจริง
2. Curvature ทนการบีบงบดีกว่าชัดเจน: 15k→5k เสีย PSNR แค่ 0.54 dB ขณะ random เสีย 0.80 dB
3. **Deliverable สำเร็จ:** โมเดล buddha **5,000 faces เป๊ะ** (verify จาก checkpoint แล้ว) ที่ `output/diffsoup_buddha_curv_d4_f5k/` — ใบหน้ายังคมชัด ฉากหลังหยาบ = งบสามเหลี่ยมถูกจัดสรรไปที่รายละเอียดจริง ๆ
4. เวลารัน 6 ตัว × ~6–8 นาที — ทั้งชุดการทดลอง S2 จบในคืนเดียว

**Caveat สำหรับรายงานฉบับจริง:** seed เดียว/scene เดียว — S3 ควรรันซ้ำ ≥3 seeds + รายงาน mean±SD และเพิ่ม LPIPS ตาม KPI
